...
Charm code is always formatted, linted, and staticly type checked before commiting. Format the code using `tox -e format` and run the linting and type checking using `tox -e lint`. Charms always have a CI workflow that ensures that code is correctly formatted and has no linting errors.
...
* CI: Running `tox -e lint`, `tox -e unit`, and `tox -e integration`
...
* Never catch `Exception`, and always keep the amount of code in `try`/`except` blocks as small as possible.
